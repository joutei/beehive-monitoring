...
// Adam Joutei : Time correction changes 
unsigned long timeCorrection = 0;

void osTimeCorrection(unsigned long x) {
    timeCorrection += x * 1000;
}
// Adam Joutei : End of changes
...
u4_t hal_ticks () {
    ...
    unsigned long newMicros = micros() + timeCorrection;

    // Scaled down timestamp. The top US_PER_OSTICK_EXPONENT bits are 0,
    // the others will be the lower bits of our return value.
    uint32_t scaled = newMicros >> US_PER_OSTICK_EXPONENT;
